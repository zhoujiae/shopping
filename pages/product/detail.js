...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (status == 1) {
          ...
          if (that.data.goodsImg.length != 0) {
            var imgsArr = that.data.goodsImg;
            imgsArr.unshift(r[0].g_img);
            r[0].g_img = imgsArr;
            that.setData({
              bannerItem: r[0].g_img,
            })
          } else {
            var rr = [];
            rr.push(r[0].g_img);
  ...
  // 商品图库
  goodsImages: function () {
    var that = this;
    wx.request({
      url: con.Index_getgoodsphotos,
      method: 'GET',
      data: {
